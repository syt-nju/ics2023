...
//to solve string, more needed  to do
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm;); 
//to solve add-longlong
...
//to solve max
  ...
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s -> dnpc += (unsigned)src1 >= (unsigned)src2 ? imm - 4: 0);
//to solve prime
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (int32_t)src1 % (int32_t)src2);//remainder
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = src1 % src2);
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = (int32_t)src1 << imm);
